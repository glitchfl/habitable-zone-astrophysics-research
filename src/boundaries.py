...
from model import distance_for_temp
...
def compute_boundaries(sample: Sample, scenarios: Scenarios, config: Config) -> Boundaries:
    ...
    return Boundaries(
        a_in=distance_for_temp(teff_column, radius_column_m, config.T_hot, albedo_row, epsilon_row) / config.AU,
        a_out=distance_for_temp(teff_column, radius_column_m, config.T_cold, albedo_row, epsilon_row) / config.AU,
    )
